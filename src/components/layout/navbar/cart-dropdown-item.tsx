  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-2">
        <h1 className="whitespace-nowrap text-lg font-bold text-[#111111]">{item.product.name}</h1>
        <article className="flex flex-col gap-2">
          {/* Right Side: Price, Size, Quanity, Total  */}
          <div className="flex w-full flex-col gap-1">
            <p className="text-sm text-[#111111]">****{item.product.product_id.slice(-4)}</p>
            {/* Price and Quantity */}
            <p>
              <strong className="text-[#111111]">${total.toFixed(2)}</strong>{' '}
              {item.quantity > 1 && (
                <span className="whitespace-nowrap text-sm text-[#111111]">
      ...
      <div className="flex items-center gap-2 rounded-full bg-lime p-2 font-bold text-[#111111] shadow-md">
